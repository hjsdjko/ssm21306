		...
		<view class="container">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
																				<button v-if="isAuth('qishou','钱包')" @tap="onAcrossTap('qianbaoxinxi')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">钱包</button>
																														                    										                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','qishou');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','qishou');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('qishou', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussqishou/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('qishou', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('qishou', this.detail);
				this.$utils.msg('审核成功');
