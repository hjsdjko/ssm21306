<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="金额" prop="jine">
          <el-select v-model="ruleForm.jine" placeholder="请选择金额">
            <el-option
                v-for="(item,index) in jineOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `qianbaoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.qianbaoxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
